<%@page import="com.ncs.gsyt.core.util.HttpRequest"%>
...
$(function(){
	$.ajax({
		...
		type: 'POST',
		dataType : "json",
		url: "${ctx}/admin/userNamage!getDept_pass.do",//请求的action路径
...
function submitConfig(){
	...
	$.ajax({
			...
			cache:false,
			type: 'POST',
			url: "${ctx}/admin/userNamage!configSubmit.do?ids="+ids+"&node="+node,//请求的action路径
			error: function () {//请求失败处理函数
				alert('请求失败');
			},
			success:function(data){ //请求成功后处理函数。 
...
<div class="guide">当前位置：用户管理-设置区域</div>
<br/>
<form action="${ctx}/admin/userNamage!configSubmit.do" id="form1" method="post">
