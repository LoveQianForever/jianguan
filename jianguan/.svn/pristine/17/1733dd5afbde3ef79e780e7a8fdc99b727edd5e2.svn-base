...
@Namespace("/admin")
@Controller
public class EnterpriseAction1 extends BaseAction implements ModelDriven<Enterprise>{
	...
	@Action(value = "/enterprise1", results = {
			...
			@Result(name = "modifybase", location = "/admin/enterprise/seedEnterpriseedit1.jsp")//种子经营者企业 信息修改
	})
		...
		if (null != list) {
			for (int i = 0; i < list.size(); i++) {
				...
				row.put("cell",
						...
						DateUtil.formatDate(obj.getCreateTime(), "yyyy-MM-dd HH:mm:ss"),
						obj.getHomePage(),
	 ...
	 * 
	 * **/
	public String  getEnter() throws Exception
	{
		/*HttpSession session=httpServletRequest.getSession(true);
		user=(User)session.getAttribute("ADMIN_USER");
	...
	public String saveModify()throws Exception
	{
		...
		String contactPerson=httpServletRequest.getParameter("contactPerson");
		String contactTelephone=httpServletRequest.getParameter("contactTelephone");
		String fax=httpServletRequest.getParameter("fax");
		...
		String enterpriseDescription=httpServletRequest.getParameter("enterpriseDescription");
		enterprise.setContactPerson(contactPerson);
		enterprise.setContactTelephone(contactTelephone);
		...
		enterprise.setHomePage(homePage);
