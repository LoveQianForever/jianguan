...
import com.ncs.gsyt.core.javapns.notification.exceptions.*;
...
public class PushNotificationManager {
	...
	public List<ResponsePacket> sendNotification(List<PushedNotification> pnl) {
		logger.info("RR批量推送时消息体的大小为：" + pnl.size());  
        ...
        while (counter < pnl.size()) {  
            try {  
                ...
                // 处理读回写数据的异常  
                try {  
                    logger.info("检查流数据是否可用:" + this.socket.getInputStream().available());  
                    ...
                    logger.debug("消息推送成功，无任何返回!", ste);  
                } catch (IOException e) {  
                    logger.debug("消息推送成功，关闭连接流时出错!", e);  
                }  
